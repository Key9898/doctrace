...
function ToastItem({
  ...
  const progressClass =
    durationMs > 5000 ? "dt-toast-progress-30" : "dt-toast-progress";

  return (
    <div className="animate-in fade-in slide-in-from-bottom-5 pointer-events-auto relative w-full overflow-hidden rounded-[2rem] border border-white/80 bg-white/70 p-4 shadow-[0_20px_50px_rgba(0,0,0,0.1)] backdrop-blur-xl transition-all dark:border-white/10 dark:bg-slate-900/80 dark:shadow-[0_20px_50px_rgba(0,0,0,0.3)]">
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1 py-0.5">
          ...
          {toast.actionLabel && toast.onAction ? (
            <button
              className="mt-2 rounded-lg border border-slate-200 bg-white px-2.5 py-1 text-[0.7rem] font-bold text-slate-800 hover:bg-slate-50 dark:border-white/15 dark:bg-slate-800 dark:text-slate-100 dark:hover:bg-slate-700"
              onClick={() => {
                toast.onAction?.();
                onDismiss(toast.id);
              }}
      ...
      <div
        className={`absolute bottom-0 left-0 h-1 ${progressClass} ${
          toast.tone === "success"
            ? "bg-emerald-500"
            : toast.tone === "error"
              ? "bg-rose-500"
