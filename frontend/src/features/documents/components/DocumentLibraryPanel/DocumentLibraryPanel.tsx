...
interface DocumentLibraryPanelProps {
  ...
  onRename: (documentId: string, fileName: string) => void;
...
const STATUS_KEYS: Record<ParserStatus, TranslationKey> = {
  parsed: "import.statusParsed",
  error: "import.statusError",
  parsing: "import.statusParsing",
  idle: "import.statusIdle",
};

function DocumentThumb({ document }: { document: ParsedDocument }) {
  if (document.sourceKind === "image" && document.objectUrl) {
    return (
      <img
        alt=""
  ...
  return (
    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-slate-100 dark:bg-white/5">
...
function DocumentCard({
  ...
  const showDownload = canDownloadEvidence(document);
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          {isRenaming ? (
            <input
              aria-label={t("import.rename")}
              autoFocus
              className="dt-input py-2"
      ...
      {document.error ? (
        <p
          ...
          role="alert"
        >
          {document.error === UNSUPPORTED_FILE_TYPE
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        {showDownload ? (
          <button
            ...
            title={
              isStoredCopyHint(document)
                ? t("import.downloadStoredHint")
                : undefined
            }
            type="button"
          >
