...
import { toColumnLetter } from "@/lib/excel";
import { createId } from "@/lib/id";
import { isAmountWithinTolerance } from "@/features/matching/services/amount-tolerance";
...
function scoreBankEntries(
...
): { match: BankMatch; matchedFields: string[] } | undefined {
  const invoiceCandidateValue = config.invoiceNumberColumnId
    ? rowValues[config.invoiceNumberColumnId]
    : "";
  const amountCandidateValue = config.amountColumnId
  ...
  const weights = config.scoreWeights ?? DEFAULT_SCORE_WEIGHTS;
  let best: { match: BankMatch; matchedFields: string[] } | undefined;
...
function formatMatchToleranceSuffix(config: MatchConfig): string {
  const percent =
    typeof config.amountTolerancePercent === "number" &&
    Number.isFinite(config.amountTolerancePercent) &&
    config.amountTolerancePercent > 0
      ? config.amountTolerancePercent
      : 0;
  const amountBit =
    percent > 0
      ? `amount ±${config.amountTolerance} or ${percent}%`
      : `amount ±${config.amountTolerance}`;

  return `(${amountBit}, date ±${config.dateToleranceDays}d)`;
}
...
export function matchSingleRow(
  ...
  const bankScored = scoreBankEntries(row.values, config, bankDocuments);
  const bankCandidate = bankScored?.match;
  ...
  const toleranceSuffix = formatMatchToleranceSuffix(config);
  const explanationParts = [
    invoiceCandidate?.matchedFields.length
      ? `Invoice matched by ${invoiceCandidate.matchedFields.join(", ")} ${toleranceSuffix}`
      : `No strong invoice match (${config.requireInvoiceNumber ? "invoice number required" : "no field match"})`,
    bankCandidate
      ? `bank statement evidence identified ${toleranceSuffix}`
      : `no bank statement hit ${toleranceSuffix}`,
  ];
  ...
  const result: MatchResult = {
    ...
    matchedFields: invoiceCandidate?.matchedFields.length
      ? invoiceCandidate.matchedFields
      : undefined,
    bankMatchedFields: bankScored?.matchedFields.length
      ? bankScored.matchedFields
      : undefined,
