...
import { getReportingConfig, resetReportingConfig } from "@/lib/i18n/reporting";
...
describe("app store", () => {
  ...
  it("does not write OCR text or rawJson into localStorage engagements", () => {
    const store = useDocTraceStore.getState();
    store.upsertDocument({
      ...
      pageCount: 1,
      status: "parsed",
      extractedText: "UNIQUE_OCR_PAYLOAD_SHOULD_NOT_PERSIST",
      pages: [
        {
          pageNumber: 1,
          text: "UNIQUE_OCR_PAYLOAD_SHOULD_NOT_PERSIST",
          snippets: ["UNIQUE_OCR_PAYLOAD_SHOULD_NOT_PERSIST"],
        },
      ],
      statementEntries: [],
      rawJson: '{"secret":true}',
    ...
    const raw = window.localStorage.getItem("doctrace.engagements") ?? "";
    expect(raw).not.toContain("UNIQUE_OCR_PAYLOAD_SHOULD_NOT_PERSIST");
    expect(raw).not.toContain('{"secret":true}');
    expect(raw).toContain("ocr-doc");
  });
  ...
  describe("engagements & modules", () => {
    ...
    it("should create new engagement", () => {
      ...
      expect(state.engagements[1].currency).toBe("MMK");
      expect(state.engagements[1].ocrLanguage).toBe("mya+eng");
    });
    ...
    it("should apply reporting config when updating the active engagement", () => {
      const store = useDocTraceStore.getState();
      store.updateEngagementReporting("test-eng-1", "USD", "eng");
      const state = useDocTraceStore.getState();
      expect(state.engagements[0].currency).toBe("USD");
      expect(state.engagements[0].ocrLanguage).toBe("eng");
  ...
  it("sets and clears a snip form field tag", () => {
    const store = useDocTraceStore.getState();
    store.setSnips([
      {
        id: "snip-1",
    ...
    store.setSnipFormField("snip-1", "invoice-number");
    expect(useDocTraceStore.getState().snips[0]?.formField).toBe(
      "invoice-number",
    );

    store.setSnipFormField("snip-1", undefined);
    expect(useDocTraceStore.getState().snips[0]?.formField).toBeUndefined();
  });
});
